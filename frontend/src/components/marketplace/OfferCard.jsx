...
const OfferCard = ({ offer, showContact = true, onContact, onEdit, onFulfillItem }) => {
  return (
    <div className="card hover:shadow-lg transition-shadow">
          ...
          {offer.items?.map((item) => (
            <div key={item.item_id} className="flex items-center justify-between text-sm">
              ...
              {!showContact && onFulfillItem && item.status !== 'given' && (
                <button
                  onClick={() => onFulfillItem(offer, item)}
      ...
      <div className="border-t pt-3 mt-3">
        <div className="flex items-center justify-between text-sm">
          <div>
            <p className="font-medium text-gray-700">{offer.user?.name || 'Volunteer'}</p>
